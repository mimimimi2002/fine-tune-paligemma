...
if __name__ == "__main__":
    ...
    print(f"[INFO] seeding with {object_detection_config.SEED}...")
    set_seed(object_detection_config.SEED)
    ...
    validation_dataset = load_dataset(
        object_detection_config.DATASET_ID, split="validation"
    )
    ...
    for epoch in range(start_epoch, object_detection_config.EPOCHS):
        ...
        if (epoch + 1) % object_detection_config.EVAL_EPOCH == 0:
            validation_loss = evaluate_loss(model, validation_dataloader)
            improved = (
                validation_loss
                < best_validation_loss
                - object_detection_config.EARLY_STOPPING_MIN_DELTA
            )
            ...
            print(
                ...
                f"{object_detection_config.EARLY_STOPPING_PATIENCE} "
            ...
            if (
                ...
                >= object_detection_config.EARLY_STOPPING_PATIENCE
            ):
                print(
        ...
        if (epoch + 1) % object_detection_config.SAVE_EPOCH == 0 or stop_early:
            save_checkpoint(
                f"./checkpoints/epoch-{epoch+1}",
                model,
                processor,
                optimizer,
                epoch + 1,
    ...
    print("[INFO] the best weights are in ./checkpoints/best")
